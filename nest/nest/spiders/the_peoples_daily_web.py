...
class PeopleDailySpiderWeb(CrawlSpider):
  ...
  article_link_format = re.compile('http://en.people.cn/n/\d+/\d+')

  #Rules to handle following urls
  rules = (Rule(LxmlLinkExtractor(allow=(article_link_format), allow_domains=('en.people.cn', )),callback='parse_follow',follow=True), )
  ...
  def parse_follow(self, response):
    ...
    article_meta_information['language'] = 'en' #http://en.wikipedia.org/wiki/List_of_ISO_639-1_codes
